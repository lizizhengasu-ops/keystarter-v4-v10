...
// =============================================
// v6.1.11 License Keys Admin 閳?Queue + CSV + Transient Lock + Chunked CSV Batch
...
$sent = ks_send_email($o->get_billing_email(),"Your Keys for Order #".$o->get_id(),$m);
$o->update_meta_data("_key_email_status", $sent ? "sent" : "failed");
}
...
add_action("admin_menu", function() {
    add_submenu_page("woocommerce", "License Keys", "棣冩斀 License Keys", "manage_woocommerce", "ks-license-keys", function() {
        if (!current_user_can("manage_woocommerce")) wp_die("Access denied.");
        $tab = $_GET["tab"] ?? "queue";
        ?>
        <div class="wrap"><h1>棣冩斀 License Keys <span style="font-size:13px;font-weight:400;color:#666">v6.1.11 閳?Queue + CSV + Transient Lock + Chunked CSV</span></h1>
        <nav class="nav-tab-wrapper">
            <a href="?page=ks-license-keys&tab=queue" class="nav-tab <?php echo $tab==="queue"?"nav-tab-active":"" ?>">棣冩惖 Queue</a>
            <a href="?page=ks-license-keys&tab=csv" class="nav-tab <?php echo $tab==="csv"?"nav-tab-active":"" ?>">棣冩憹 CSV Batch</a>
        </nav>
        <?php
...
function ks_render_queue($nonce) {
    ...
    <div id="ks-queue-order">
        <h2>#<?php echo esc_html($oid) ?> 閳?            <span style="font-weight:400;font-size:14px"><?php echo esc_html($order->get_billing_first_name() . " " . $order->get_billing_last_name()) ?>
        ...
        <form id="ks-queue-form" method="post">
            ...
            <table style="width:100%;border-collapse:collapse;margin:12px 0">
                ...
                <tr>
                    <td style="padding:8px;border-bottom:1px solid #f0f0f0;font-size:13px">
                        <?php echo esc_html($item->get_name()) ?> <span style="color:#999">鑴??php echo $item->get_quantity() ?></span>
                    </td>
                    <td style="padding:8px;border-bottom:1px solid #f0f0f0">
                        ...
                        <?php else: ?>
                            <span style="color:#1565c0;font-size:12px">棣冩憹 Physical product 閳?ship separately</span>
                        <?php endif; ?>
                    </td>
                    <td style="text-align:center;padding:8px;border-bottom:1px solid #f0f0f0">
                        <?php if ($key): ?>
                            <span style="color:#2e7d32;font-size:11px">閴?Sent</span> <a href="?page=ks-license-keys&tab=queue&resend=1" style="font-size:10px;color:#7c3aed">Resend</a>
                        <?php elseif (!$is_virtual): ?>
                            <span style="color:#1565c0;font-size:11px">棣冩憹 Physical</span>
                        <?php else: ?>
                            <span style="color:#e65100;font-size:11px">閳?Pending</span>
                        <?php endif; ?>
                    </td>
                </tr>
                <?php endforeach; ?>
            ...
            <p style="font-size:11px;color:#999">
                閳?<kbd style="background:#f0f0f1;padding:0 4px;border:1px solid #ddd;border-radius:2px">S</kbd> Save &amp; Send
                璺?<kbd style="background:#f0f0f1;padding:0 4px;border:1px solid #ddd;border-radius:2px">N</kbd> Next Order
                璺?Keys auto-validated, no duplicates allowed
            </p>
            ...
            <a href="?page=ks-license-keys&tab=queue&oid=<?php echo $pending[min($idx+1,$total-1)] ?>" class="button" style="margin-left:4px">Skip 閳?Next</a>
        </form>
    </div></div>
    <script>
...
function ks_render_csv($nonce) {
    ...
    <div style="display:flex;gap:20px;margin-top:12px;align-items:stretch">
        ...
        <div style="flex:1;background:#fff;border:1px solid #c3c4c7;border-radius:6px;padding:16px">
            <h3>棣冩憶 Upload CSV with Keys</h3>
    ...
    <div style="margin-top:20px;background:#fff;border:1px solid #c3c4c7;border-radius:6px;padding:12px;font-size:11px;color:#666">
        閴?<strong>v6.1.11 safeguards active:</strong>
        CSRF nonce 璺?XSS esc_html 璺?Key dedup check 璺?Order-level lock 璺?Batch email (1 per order)
    </div>
    <?php
}
...
add_action("wp_dashboard_setup",function(){wp_add_dashboard_widget("ks_pending","棣冩斀 Pending Keys",function(){
...
echo "<a href=\"admin.php?page=ks-license-keys\" class=\"button\">Go 閳?/a>";});});



// v7.0: Cookie-based auth check + orders in one endpoint (bypasses rest_authentication_errors)
add_action("rest_api_init", function() {
  register_rest_route("keystarter/v1", "/customer-account", [
    "methods" => "GET",
    "callback" => function() {
      foreach ($_COOKIE as $name => $value) {
        if (strpos($name, "wordpress_logged_in_") === 0) {
          ...
          if ($uid) {
            ...
            $orders = wc_get_orders(["customer" => wp_get_current_user()->user_email, "limit" => 50, "orderby" => "date", "order" => "DESC"]);
            $data = [];
            foreach ($orders as $o) {
              $items = [];
              foreach ($o->get_items() as $item) {
                $p = wc_get_product($item->get_product_id());
                $items[] = ["name" => $item->get_name(), "qty" => $item->get_quantity(), "virtual" => $p ? $p->get_virtual() : false];
              }
              $data[] = ["id" => $o->get_id(), "date" => $o->get_date_created() ? $o->get_date_created()->format("Y-m-d H:i") : "", "status" => $o->get_status(), "total" => (float)$o->get_total(), "items" => $items, "key_email_status" => $o->get_meta("_key_email_status") ?: "pending"];
            }
            return ["ok" => true, "logged_in" => true, "orders" => $data, "logout_url" => wp_logout_url("/my-account/")];
          }
        }
      }
      return ["ok" => true, "logged_in" => false, "orders" => []];
    },
    "permission_callback" => "__return_true"
  ]);
});

// v2.0: My Account Skin + SPA-style Navigation Bar
add_action("wp_enqueue_scripts", function() {
    if (is_account_page()) {
        wp_register_style("ks-myaccount-skin", false);
        wp_enqueue_style("ks-myaccount-skin");
        wp_add_inline_style("ks-myaccount-skin", "
.woocommerce-account { background:#f5f5f7 !important; }
.site-header,.ast-desktop-header,.ast-mobile-header,
.ast-primary-header-bar,.main-header-bar,
.ast-below-header,.ast-above-header,
.site-footer,.entry-header,.ast-title-bar,
...
.ks-nav-links { display:flex; gap:8px; align-items:center; }
.ks-nav-link {
  font-size:13px; font-weight:500; color:#86868b;
  text-decoration:none; padding:6px 14px;
  border-radius:8px; transition:all 0.2s;
}
.ks-nav-link:hover { color:#1d1d1f; background:#f5f5f7; }
.ks-nav-link.active { color:#fff !important; background:#7c3aed !important; }
.woocommerce-MyAccount-content {
  background:#fff; border:1px solid #e8e8ed; border-radius:16px; padding:32px;
}
...
.woocommerce-MyAccount-navigation ul li { margin:0; padding:0; flex:0 0 auto; }
...
.woocommerce-MyAccount-navigation ul li a:hover { color:#1d1d1f; background:#f5f5f7; }
.woocommerce-MyAccount-navigation ul li.is-active a { color:#fff; background:#7c3aed; }
...
.woocommerce button.button:hover,.woocommerce a.button:hover { background:#6d28d9 !important; }
.woocommerce form .form-row .input-text,.woocommerce form .form-row input {
  width:100%; padding:12px 14px; border:1px solid #e8e8ed;
  border-radius:12px !important; font-size:14px; color:#1d1d1f;
  background:#fff;
}
.woocommerce form .form-row .input-text:focus { border-color:#7c3aed; box-shadow:0 0 0 3px rgba(124,58,237,0.1); }
.woocommerce-orders-table { width:100%; border-collapse:collapse; }
.woocommerce-orders-table th { padding:12px 16px; color:#86868b; font-size:12px; text-transform:uppercase; border-bottom:1px solid #f0f0f2; }
.woocommerce-orders-table td { padding:14px 16px; border-bottom:1px solid #f5f5f7; }
.woocommerce-message { background:#f0fdf4; font-size:14px; color:#166534; border-radius:12px; padding:14px 18px !important; }
.woocommerce-info { background:#eff6ff; color:#1e40af; font-size:14px; border-radius:12px; padding:14px 18px !important; }
.woocommerce-error { background:#fef2f2; color:#991b1b; font-size:14px; border-radius:12px; padding:14px 18px !important; }
.woocommerce-Address { background:#fff; border:1px solid #e8e8ed; border-radius:12px; padding:20px; }
@media (max-width:768px) {
  .woocommerce-MyAccount-navigation ul { flex-direction:column; }
  .ks-nav-bar { padding:0 16px; }
  .ks-nav-inner { height:48px; }
}
");
    }
}, 100);

// v2.0: Inject SPA-style nav bar HTML
add_action("wp_body_open", function() {
    if (!is_account_page()) return;
    echo "<div class=\"ks-nav-bar\"><div class=\"ks-nav-inner\"><a href=\"/\" class=\"ks-nav-brand\">KeyStarter</a><div class=\"ks-nav-links\"><a href=\"/\" class=\"ks-nav-link\">Home</a><a href=\"/cart/\" class=\"ks-nav-link\">Cart</a><a href=\"/my-account/\" class=\"ks-nav-link active\">Account</a></div></div></div>";
});
