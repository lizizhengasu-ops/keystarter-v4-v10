...
(function () {
  if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
  // terminal motion signature: strata rows, contour signal, glare sweep
  document.querySelectorAll(".slab-card, .strata-row, .ledger-row, .prod-card").forEach(function (el) {
  ...
  document.querySelectorAll(".slab-card, .strata-row, .ledger-row, .prod-card").forEach(function (el) { io2.observe(el); });
