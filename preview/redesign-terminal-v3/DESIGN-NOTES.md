# KeyStarter Terminal v3.0 - TERMINAL STYLE REBUILD

## 版本命名
- 产品名：**KeyStarter Terminal v3.0**
- 风格代号：Terminal（Linear-style dark technical console）
- 按 **replica-kit-v2.0 / v2.0-design-differentiation** 全量重建；保留全部 21 个页面与全部功能组件。

## Design Read
Design Read：Reading this as a high-trust authorized Microsoft license storefront with a Linear-like dark telemetry console language, leaning toward Sora + JetBrains Mono + violet.
...
推荐：Linear Terminal —— 为什么最适配：终端/遥测把“授权发行”变成可观测的工程体验，对企业采购和开发者受众最有辨识度。

## 调色板轮换
- 家族：Neon-Chrome 暗底（violet variant）；依据 palette-rotation.md 选未被最近 3 站使用家族，避开 beige+brass 默认。

## 卡片形态
- cardVariant：slab-grid / strata-rail；与 data-layout-model 的 PRODUCT 声明一致。

## Chrome 签名
- 搜索：KS:: 终端搜索条 + 深色面板。
- 购物车：License queue 抽屉。
- 年龄门/聊天：保留品牌化标准件。

## 工艺签名
- telemetry-map（Hero 终端 + 遥测波形 SVG）：data-craft 手写内联 SVG。
- signal-grid（Ecosystem 区信号网格 SVG）：data-craft 手写内联 SVG。
...
## impeccable critique
impeccable critique 结论：Terminal 的深色遥测语言原创性强，终端 Hero 与 slab/strata 商品形态不撞款；低频子页面保留功能模板，视觉由新主题统一。
