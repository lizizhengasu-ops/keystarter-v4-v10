# KeyStarter Editorial v5 - EDITORIAL STYLE REBUILD

## 版本命名
- 产品名：**KeyStarter Editorial v5**
- 风格代号：Editorial（Editorial Quiet Luxury 编辑式对页杂志）
- 按 **replica-kit v2.x / design-differentiation v2.x** 全量重建；保留全部 21 个页面与全部功能组件；本地预览，禁止移植生产前台。

## Design Read
Design Read：Reading this as an editorial magazine for software licensing: Bodoni display type, asymmetric spreads, quiet luxury and a red accent.
...
## impeccable critique
impeccable critique 结论：Editorial 的 Bodoni 衬线 + 红强调 + 对页排版形成杂志级品牌记忆；全部子页面独立重构。

## 骨架差异
- Editorial：Hero → trust strip → Manifesto → Best → New → Featured → Compare spec → Reviews → Journal → Trust → Newsletter → Support
