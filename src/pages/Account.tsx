...
export default function AccountPage() {
  // Single call: check auth + get orders if logged in
  useEffect(() => {
    fetch("/wp-json/keystarter/v1/customer-account",{credentials:"same-origin"})
      .then(r => r.json())
      .then(data => {
        if (data.logged_in) {
          setLoggedIn(true);
      ...
      .catch(() => { setLoggedIn(false); setLoadingOrders(false); setOrderError(true); });
  }, []);
  ...
  // login states removed - login uses native form POST to /wp-login.php
