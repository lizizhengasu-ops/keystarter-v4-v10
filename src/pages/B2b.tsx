...
export default function B2bPage() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const r = await fetch(api("/wp-json/keystarter/v1/send-email"), {
        method: "POST",
      ...
      if (form.phone.includes("@")) {
        fetch(api("/wp-json/keystarter/v1/send-email"), {
          ...
          body: JSON.stringify({
            ...
            message: "<p>Hi " + form.contact + ",</p><p>Thank you for your compliance quote request. Our experts will review your needs within 30 minutes.</p><p>For urgent inquiries, please email " + SITE.supportEmail + ".</p><p>Best regards,<br>" + SITE.siteName + " Compliance Team</p>"
          })
        }).catch(() => {});
      }
  ...
  return (
    <div className="bg-[#f5f5f7] text-[#1d1d1f] antialiased">
      <div id="enterprise-b2b" className="bg-gradient-to-r from-[#6d28d9] to-[#1a1a2e] text-white px-6 sm:px-12 py-20 text-center">
        ...
        <a href={"mailto:" + SITE.supportEmail + "?subject=Enterprise%20B2B%20Inquiry"} className="inline-block bg-white text-[#6d28d9] px-8 py-3 rounded-xl font-semibold text-sm no-underline hover:bg-gray-100 transition">{t("b2b.contact")}</a>
      </div>
