...
export default function App() {
  ...
  useEffect(() => {
    const id = window.requestIdleCallback(() => {
      fetchProducts().then(setApiProducts).catch(function(e){console.warn("API fetch failed:",e)});
    }, { timeout: 3000 });
...
const [activeTab, setActiveTab] = useState('all');
 ...
 const [blogPosts, setBlogPosts] = useState<any[]>([]);
  ...
  useEffect(() => {
    const id = window.requestIdleCallback(() => {
      fetch(api("/wp-json/wp/v2/posts?per_page=3"))
        .then((r) => r.json())
        .then(setBlogPosts)
        .catch(() => {});
    }, { timeout: 3000 });
  ...
  const handleB2BSubmit = async (e: any) => {
    ...
    try {
      ...
      if (ce.includes("@")) {
        fetch(api("/wp-json/keystarter/v1/send-email"), {
          method: "POST",
          headers: {"Content-Type": "application/json"},
          body: JSON.stringify({
            to: ce,
            ...
            from_name: (fd.get("contact") || ""),
            reply_to: ce,
            subject: "Thank you for your Compliance Quote - " + SITE.siteName,
            message: "<p>Hi " + (fd.get("contact") || "") + ",</p><p>Thank you for your compliance quote request. Our experts will review your needs and provide a cost-effective quote within 30 minutes.</p><p>For urgent inquiries, please email " + SITE.supportEmail + ".</p><p>Best regards,<br>" + SITE.siteName + " Compliance Team</p>"
          })
        }).catch(() => {});
      }
      ...
      showToast("Network error. Please email " + SITE.supportEmail + " directly.", "⚠️");
    }
  };

  const handleB2BWhatsApp = (e: any) => {
    const fd = new FormData(e.currentTarget.form);
    const text = "New B2B Quote Request\n\n" +
      "Company: " + (fd.get("company") || "") + "\n" +
      "Estimated Licenses: " + (fd.get("units") || "") + "\n" +
      "Primary Product Needs: " + (fd.get("product") || "") + "\n" +
      "Contact Name: " + (fd.get("contact") || "") + "\n" +
      "Phone / Email: " + (fd.get("phone") || "");
...
const filteredSkus = PREMIUM_SKUS.filter(sku =>
    inSiteCatalog(sku.id) && (activeTab === 'all' || sku.category === activeTab)
  );
            ...
            <div className="bg-white text-[#1d1d1f] p-8 rounded-2xl border border-[#e8e8ed] shadow-2xl relative">
              ...
              <form onSubmit={handleB2BSubmit} className="space-y-4">
                ...
                <div className="flex gap-3">
                  <button type="submit" className="flex-1 bg-[#7c3aed] hover:bg-[#6d28d9] text-white text-xs font-bold py-3.5 rounded-lg transition shadow-lg shadow-blue-500/10">
                    Get Free Custom Quote
                  </button>
                  <button type="button" onClick={handleB2BWhatsApp} className="flex-1 inline-flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#1faa55] text-white text-xs font-bold py-3.5 rounded-lg transition shadow-lg shadow-green-500/10">
