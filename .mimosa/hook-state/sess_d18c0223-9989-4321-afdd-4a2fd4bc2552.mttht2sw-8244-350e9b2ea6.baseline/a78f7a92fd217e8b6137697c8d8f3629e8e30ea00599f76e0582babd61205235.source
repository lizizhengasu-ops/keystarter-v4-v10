  ...
  {formStatus === "sent" ? (
    ...
    <form onSubmit={handleSubmit} className="max-w-md mx-auto text-left space-y-4">
      <div style={{position:"absolute",left:"-9999px"}} aria-hidden="true">
        <input name="website" tabIndex={-1} aria-hidden="true" value={form.honeypot_website} onChange={e=>setForm({...form,honeypot_website:e.target.value})} />
      </div>
              <input type="text" name="name" aria-label="Name" placeholder="Name" value={form.name} onChange={e=>setForm({...form,name:e.target.value})} required autoFocus
        className="w-full p-3 border border-[#e8e8ed] rounded-xl text-sm" />
      {errors.name && <p className="text-red-500 text-xs -mt-2">{errors.name}</p>}
      <input type="email" name="email" aria-label="Email" placeholder="Email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} required
        className="w-full p-3 border border-[#e8e8ed] rounded-xl text-sm" />
      {errors.email && <p className="text-red-500 text-xs -mt-2">{errors.email}</p>}
      <input type="tel" name="phone" aria-label="Phone" placeholder="Phone (optional)" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})}
                className="w-full p-3 border border-[#e8e8ed] rounded-xl text-sm" />
              <input type="text" name="subject" aria-label="Subject" placeholder="Subject" value={form.subject} onChange={e=>setForm({...form,subject:e.target.value})} required
                className="w-full p-3 border border-[#e8e8ed] rounded-xl text-sm" />
      <textarea name="message" aria-label="Message" placeholder="Message" value={form.message} onChange={e=>setForm({...form,message:e.target.value})} required rows={4}
